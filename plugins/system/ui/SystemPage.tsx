  ...
  return (
    <div className={`rounded-lg border transition-colors ${
      ...
      >
        ...
        {network.in_use && <CheckCircle size={14} className="text-emerald-400 flex-none" />}
        {!network.in_use && (
          expanded
...
// ── Confirm button (click once to arm, again to confirm) ──────────────────────

function ConfirmButton({
  label, confirmLabel, icon: Icon, onClick, variant = 'danger', disabled = false,
}: {
  label: string
  confirmLabel: string
  icon: React.ElementType
...
export function SystemPage() {
  ...
  useEffect(() => {
    loadCore()
    const id = setInterval(loadCore, 5000)
  ...
  const handleLoadTimezones = async () => {
    ...
    try {
      setTimezones(await api.listTimezones())
    } catch {
      setTimezones([])
    }
  }
              ...
              <div key={disk.mountpoint}>
                <div className="flex items-center justify-between text-xs mb-1.5">
                  <div className="flex items-center gap-2">
                    <span className="text-slate-300 font-mono">{disk.mountpoint}</span>
                    {disk.removable && (
                      <span className="px-1.5 py-0.5 rounded text-[10px] bg-sky-500/15 text-sky-400 border border-sky-500/30">USB</span>
                    )}
                  </div>
                ...
                <div className="h-1.5 bg-surface-overlay rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full ${
                      ...
                      disk.percent >= 75 ? 'bg-amber-500' : 'bg-emerald-500'
                    }`}
                    style={{ width: `${Math.min(100, disk.percent)}%` }}
                  />
                </div>
                <p className="text-[10px] text-slate-600 mt-1">{disk.filesystem} · {disk.device}</p>
              </div>
            ))}
          </div>
        </Section>
